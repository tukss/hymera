...
PetscErrorCode Computepsi(TS ts, PetscInt step, Vec X, void *ptr)
{
  PetscLogEvent  USER_EVENT;
  PetscClassId   classid;
  PetscLogDouble user_event_flops;
  ...
  PetscClassIdRegister("class name",&classid);
  PetscLogEventRegister("Computepsi",classid,&USER_EVENT);
  PetscLogEventBegin(USER_EVENT,0,0,0,0);
  ...
  TSGetDM(ts,&da);
    ...
    //for (k = 0; k < user->Nz; k++) {
      //for (j = 0; j < user->Nphi; j++) {
        //for (i = 0; i < user->Nr; i++) {
          //vecCR[(k*user->Nr*(user->Nphi)+j*user->Nr+i)] = g_P[3*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)];
          //vecCZ[(k*user->Nr*(user->Nphi)+j*user->Nr+i)] = g_P[2+3*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)];
          //PetscPrintf(PETSC_COMM_WORLD, "The (r,z) cell center coordinates at global index %d + %d*Nr + %d*Nr*Nphi are (%g,%g)\n", i, j, k, (double)vecCR[(k*user->Nr*(user->Nphi)+j*user->Nr+i)], (double)vecCZ[(k*user->Nr*(user->Nphi)+j*user->Nr+i)]);
          //PetscPrintf(PETSC_COMM_WORLD, "The difference between (r,z) cell center coordinates at global index %d + %d*Nr + %d*Nr*Nphi is (%g,%g)\n", i, j, k, (double)(vecCR[(k*user->Nr*(user->Nphi)+j*user->Nr+i)] - g_P[3*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)]), (double)(vecCZ[(k*user->Nr*(user->Nphi)+j*user->Nr+i)] - g_P[2+3*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)]));
        //}
      //}
    //}
    ...
    //for (k = 0; k < user->Nz; k++) {
      //for (j = 0; j < user->Nphi; j++) {
        //for (i = 0; i < user->Nr+1; i++) {
            //PetscPrintf(PETSC_COMM_SELF, "The (r,phi,z) r-face center coordinates at global index %d + %d*(Nr+1) + %d*(Nr+1)*Nphi are (%g,%g,%g)\n", i, j, k, (double)(g_R)[3*(k*(user->Nr+1)*user->Nphi+j*(user->Nr+1)+i)], (double)(g_R)[1+3*(k*(user->Nr+1)*user->Nphi+j*(user->Nr+1)+i)], (double)(g_R)[2+3*(k*(user->Nr+1)*user->Nphi+j*(user->Nr+1)+i)]);
        //}
      //}
    //}
      ...
      /* Loop over the patch of the entire domain */
    //for (k = 0; k < user->Nz; k++) {
      //for (j = 0; j < user->Nphi+1; j++) {
        //for (i = 0; i < user->Nr; i++) {
            //PetscPrintf(PETSC_COMM_SELF, "The (r,phi,z) phi-face center coordinates at global index %d + %d*Nr + %d*Nr*(Nphi+1) are (%g,%g,%g)\n", i, j, k, (double)g_P[3*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)], (double)g_P[1+3*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)], (double)g_P[2+3*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)]);
        //}
      //}
    //}

      /* Loop over the patch of the entire domain */
    //for (k = 0; k < user->Nz+1; k++) {
      //for (j = 0; j < user->Nphi; j++) {
        //for (i = 0; i < user->Nr; i++) {
            //PetscPrintf(PETSC_COMM_SELF, "The (r,phi,z) z-face center coordinates at global index %d + %d*Nr + %d*Nr*Nphi are (%g,%g,%g)\n", i, j, k, (double)g_Z[3*(k*user->Nr*user->Nphi+j*user->Nr+i)], (double)g_Z[1+3*(k*user->Nr*user->Nphi+j*user->Nr+i)], (double)g_Z[2+3*(k*user->Nr*user->Nphi+j*user->Nr+i)]);
        //}
      //}
    //}
    ...
    //for (k = 0; k < user->Nz; k++) {
      //for (j = 0; j < user->Nphi; j++) {
        //for (i = 0; i < user->Nr+1; i++) {
            //PetscPrintf(PETSC_COMM_SELF, "The r-face center value at global index %d + %d*(Nr+1) + %d*(Nr+1)*Nphi is: %g\n", i, j, k, (double)gf_BR[1*(k*(user->Nr+1)*user->Nphi+j*(user->Nr+1)+i)]);
        //}
      //}
    //}
    ...
    //for (k = 0; k < user->Nz; k++) {
      //for (j = 0; j < user->Nphi+1; j++) {
        //for (i = 0; i < user->Nr; i++) {
            //PetscPrintf(PETSC_COMM_SELF, "The phi-face center value at global index %d + %d*Nr + %d*Nr*(Nphi+1) is: %g\n", i, j, k, (double)gf_BP[1*(k*user->Nr*(user->Nphi+1)+j*user->Nr+i)]);
        //}
      //}
    //}
    ...
    //for (k = 0; k < user->Nz+1; k++) {
      //for (j = 0; j < user->Nphi; j++) {
        //for (i = 0; i < user->Nr; i++) {
            //PetscPrintf(PETSC_COMM_SELF, "The z-face center value at global index %d + %d*Nr + %d*Nr*Nphi is: %g\n", i, j, k, (double)gf_BZ[1*(k*user->Nr*user->Nphi+j*user->Nr+i)]);
        //}
      //}
    //}
      ...
      for (int j = 0; j < user->Nz; ++j){
        for (int i = 0; i < user->Nr; ++i){
          g_P[0 + 3*i + 3*user->Nr*user->Nphi + j*3*user->Nr*(user->Nphi+1)] = g_P[0 + 3*i + 3*user->Nr*0 + j*3*user->Nr*(user->Nphi+1)];
          g_P[1 + 3*i + 3*user->Nr*user->Nphi + j*3*user->Nr*(user->Nphi+1)] = g_P[1 + 3*i + 3*user->Nr*0 + j*3*user->Nr*(user->Nphi+1)];
          g_P[2 + 3*i + 3*user->Nr*user->Nphi + j*3*user->Nr*(user->Nphi+1)] = g_P[2 + 3*i + 3*user->Nr*0 + j*3*user->Nr*(user->Nphi+1)];
          gf_BP[i + user->Nr*user->Nphi + j*user->Nr*(user->Nphi+1)] = g_P[i + user->Nr*0 + j*user->Nr*(user->Nphi+1)];
        }
      }

      //PetscPrintf(PETSC_COMM_WORLD, "Before call to multiplybyR.\n");
      multiplybyR(gf_BR, g_R, (user->Nr+1)*(user->Nz)*(user->Nphi));
      multiplybyR(gf_BP, g_P, (user->Nr)*(user->Nz)*(user->Nphi+1));
      multiplybyR(gf_BZ, g_Z, (user->Nr)*(user->Nz+1)*(user->Nphi));

      int nRR, nRZ, nZR, nZZ;
      int nStencilWidth = 5;
      int bIntersect = 1;

      //PetscPrintf(PETSC_COMM_WORLD, "Before call to createHermiteFD.\n");
      PetscScalar * hp_BR = createHermiteFD(&nRR, &nZR, gf_BR, g_R, nStencilWidth, bIntersect, user->Nr+1, user->Nphi, user->Nz);
      PetscScalar * hp_BP = createHermiteFD(&nRZ, &nZR, gf_BP, g_P, nStencilWidth, bIntersect, user->Nr, user->Nphi+1, user->Nz);
      PetscScalar * hp_BZ = createHermiteFD(&nRZ, &nZZ, gf_BZ, g_Z, nStencilWidth, bIntersect, user->Nr, user->Nphi, user->Nz+1);
      PetscScalar * hp_RR = (PetscScalar*) malloc(sizeof(PetscScalar)*nRR);
      PetscScalar * hp_RZ = (PetscScalar*) malloc(sizeof(PetscScalar)*nRZ);
      PetscScalar * hp_ZR = (PetscScalar*) malloc(sizeof(PetscScalar)*nZR);
      PetscScalar * hp_ZZ = (PetscScalar*) malloc(sizeof(PetscScalar)*nZZ);

      for (int i = 0; i < nRR; ++i){
        hp_RR[i] = hp_BR[9 + 12*i];
      }

      for (int i = 0; i < nRZ; ++i){
        hp_RZ[i] = hp_BZ[9 + 12*i];
      }

      for (int i = 0; i < nZR; ++i){
        hp_ZR[i] = hp_BR[11 + 12*i*nRR];
      }
      for (int i = 0; i < nZZ; ++i){
        hp_ZZ[i] = hp_BZ[11 + 12*i*nRZ];
      }

      PetscScalar hR = hp_RR[1] - hp_RR[0];
      PetscScalar hZ = hp_ZZ[1] - hp_ZZ[0];

      PetscScalar * hd_BR = (PetscScalar*) malloc(sizeof(PetscScalar)*36*(nRR-1)*(nZR-1));
      PetscScalar * hd_BP = (PetscScalar*) malloc(sizeof(PetscScalar)*36*(nRZ-1)*(nZR-1));
      PetscScalar * hd_BZ = (PetscScalar*) malloc(sizeof(PetscScalar)*36*(nRZ-1)*(nZZ-1));

      //PetscPrintf(PETSC_COMM_WORLD, "Before call to interpolateHermite.\n");
      interpolateHermite(hp_BR, hd_BR, nRR, nZR, user);
      interpolateHermite(hp_BP, hd_BP, nRZ, nZR, user);
      interpolateHermite(hp_BZ, hd_BZ, nRZ, nZZ, user);

      PetscScalar* hd_psi_iz = (PetscScalar*) malloc(sizeof(PetscScalar)*42*(nRR-1)*(nZR-1));
      //PetscScalar* hd_chi_iz = malloc(sizeof(PetscScalar)*42*(nRZ-1)*(nZR-1));
      PetscScalar* hd_psi_ir = (PetscScalar*) malloc(sizeof(PetscScalar)*7* (nRZ-1));

      //PetscPrintf(PETSC_COMM_WORLD, "Before call to integrateHermite.\n");
      integrateHermite_2D_z(hd_BR, hd_psi_iz, nRR, nZR, hR, hZ);
      //integrateHermite_2D_z(hd_BP, hd_chi_iz, nRZ, nZR, hR, hZ);
      integrateHermite_1D_r(hd_BZ, hd_psi_ir, nRZ, nZR, hR, hZ);

      tHermiteDivFreeFields fields;
      fields.m_hd_psi_iz = hd_psi_iz;
      //fields.m_hd_chi_iz = hd_chi_iz;
      fields.m_hd_psi_ir = hd_psi_ir;
      fields.m_hp_RR = hp_RR;
      fields.m_hp_ZR = hp_ZR;
      fields.m_hp_RZ = hp_RZ;
      fields.m_hp_ZZ = hp_ZZ;
      fields.m_nRR = nRR;
      fields.m_nZR = nZR;
      fields.m_nRZ = nRZ;
      fields.m_nZZ = nZZ;

      PetscScalar * psi = (PetscScalar*) malloc(sizeof(PetscScalar)*(user->Nr)*(user->Nz)*(user->Nphi));

      //PetscPrintf(PETSC_COMM_WORLD, "Before call to evalPsi.\n");
      evalPsi(psi, vecCR, vecCZ, (user->Nr)*(user->Nz)*(user->Nphi), fields);
      //PetscPrintf(PETSC_COMM_WORLD, "After call to evalPsi.\n");
      ...
      //PetscPrintf(PETSC_COMM_WORLD, "Before call to DumpPsi_Cell.\n");
      DumpPsi_Cell(ts, step, psi, user);
      free(psi);
